...
const fetchPokemons = () => {
  fetch('https://pokeapi.co/api/v2/pokemon/?limit=50')
    ...
    .catch((error) => console.log('uncought error:', error))
}
fetchPokemons()
...
const fetchBulbasaurData = () => {
  fetch('https://pokeapi.co/api/v2/pokemon/1/')
    ...
    .then((bulbasaur) => {
      ...
      image.src = bulbasaur.sprites.front_default
      nameOfPokemon.innerHTML = `${bulbasaur.name} `
      weight.innerHTML = `${bulbasaur.weight}`
      height.innerHTML = `${bulbasaur.height}`
      const typesNames = bulbasaur.types.map((type) => type.type.name)
